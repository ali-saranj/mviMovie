...
class MoviesRepositoryImpl @Inject constructor(private val dataSourceImpl: MoviesDataSourceImpl) :
    MoviesRepository {

    override fun listMovies(): Flow<Resource<List<Movie>>> = flow {
        when (val response = dataSourceImpl.listMovies()) {
